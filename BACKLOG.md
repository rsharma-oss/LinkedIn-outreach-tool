...
- **Status:** ✅ **UN-GATED June 28 — the customizer is LIVE on `icp-finder.html`.** Two-list + rank×domain engine ported into the live page (live storage keys `ga_icp_kw`/`ga_icp_exact` via `window.ICP_K`); ✏️ Customize button + full modal (Pick-from-network + Edit-lists tabs, vertical presets, Save/Load config) + Willis help published. Verified end-to-end in preview (presets apply + relabel, live storage, zero console errors). Packaging: customizer **free**, paid = the done-for-you **service** (muted CTA, no in-app prices). _Follow-up: **inline `icp-picker.js` + `icp-profiles.js` into the offline bundle** (the offline icp-finder currently lacks the customizer — would 404 those scripts; mirror the Willis inlining in `build_offline_bundle.py`)._
