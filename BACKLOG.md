...
- **Prototype (`icp-finder-prototype.html` + `icp-picker.js`, unlisted) — multi-select picker built:** Titles tab (pick titles → T1/T2/T3) + **Companies tab** (pick companies → T2 ecosystem). Isolated storage (`ga_icp_kw_proto`), so it can't touch the live app's `ga_icp_kw`.
