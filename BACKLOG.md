...
- **Status:** ✅ **Shipped June 22 — both Fix A and Fix B.**
- **What shipped:** load-screen + `.icp-note` now state the ICP is **tuned for Shopify DTC** (customize, or we'll build yours); the headline stat shows **real coverage + an "unmatched" count** (fixed a bug where `s-total-sub` always read ~100%); a **"✏️ Customize keywords" in-app editor** lets users edit the T1/T2/T3 keyword lists, persists to `localStorage['ga_icp_kw']`, and **re-tiers the whole network live** (+ the Playbook via `ga_icp_data`) — with per-tier reset, reset-all, a "Custom" badge, and live match counts. Engine: keyword lists are now editable arrays compiled to regex (`compileKw`); `classifyAll()` / `reclassify()` drive live re-classification. EXCL noise-filter kept fixed.
- _Original framing retained below for reference._
- **(was) Not started** — elevated as the ICP Finder's central limitation.
