...
- `:root` = dark defaults; `[data-theme="light"]` on `<html>` overrides them. **Dark values are unchanged** from pre-theme — don't "tidy" them. **WCAG exception (July 15 2026):** muted-text tokens were raised for AA compliance — dark `--tx3`/`--nav-idle`/`--tab-idle`/`--text-muted`/footer links ≥ **0.55 alpha**; light equivalents ≥ **0.62**; light `--li2:#0369a1` (dark keeps #00a0dc). New colored TEXT must clear **4.5:1** (3:1 ≥24px) in BOTH themes — the light-shade status colors (mint/amber/lavender/#93c5fd family) are dark-only; their light counterparts live in the appended "WCAG light-mode overrides" blocks + `--li*`/`--warn`/`--ok`/`--dlgreen` vars. Run a contrast audit after any palette change.
...
- **`WILLIS_CONFIG` — the engine is now site-neutral (July 4 2026).** `willis.js` reads **`window.WILLIS_CONFIG`** (set BEFORE it loads — LinkVault's lives at the top of `willis-articles.js`): `{name, productName, tagline, placeholder, artPath, reportEmail, actions:[{label, href|action:'report', primary}]}` — `name` is the ASSISTANT (Willis), **`productName` is the PRODUCT (LinkVault)**, used in report subjects (`[LinkVault — issue]`); falls back to document.title. `renderIdle()` builds the panel's action buttons from `CFG.actions` (LinkVault ships "📅 Book a demo →" → demo.html + "🐞 Report a bug" → gaReport) — these are where the old nav Book Demo + Report went; the engine itself has **no LinkVault data** (neutral defaults: no actions, generic placeholder), so the willis-kit is genuinely drop-in for other apps. `gaReport` uses `CFG.reportEmail`. ⚠️ Keep site data in `WILLIS_CONFIG`/articles, never in `willis.js`.

**Portability contract (Aug 3 2026) — the engine must stay product-neutral.**
- **No default report address.** `gaReport` uses `CFG.reportEmail` and has **no fallback**. It previously fell back to `rahul@growthautomated.ai`, which meant a different app dropping in the kit would have silently routed *its* users' bug reports to LinkVault's owner. With no address configured the dialog offers **copy-only** and says "send it to whoever supports this app". **Never reintroduce a default.**
- **`artPath`** (default `'willis/'`) makes the character art relocatable; image `alt` uses `CFG.name` so the character can genuinely be renamed.
- **Verified** by booting the engine on a foreign host with `{name:'Pip', productName:'OtherApp'}` and no `reportEmail`: renamed throughout, zero references to LinkVault or its owner, mail buttons correctly hidden.
- ⚠️ **The reusable kit lives OUTSIDE this repo** at `~/Desktop/Willis/willis-kit/` (engine + art + README + `.zip`) — it is **local-only and unversioned**. `willis.js` here is the source of truth; copy it to the kit after any engine change (`cp willis.js ~/Desktop/Willis/willis-kit/willis.js`). If that Desktop folder is lost, the kit packaging and its README are gone — worth moving to its own repo.
