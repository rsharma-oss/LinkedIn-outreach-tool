...
- **Panel home-view action buttons (July 4 2026):** `renderIdle()` now renders two buttons under the welcome card — **"📅 Book a demo →"** (`.wz-act-demo`, → `demo.html`) and **"🐞 Report a bug"** (`.wz-act-report`/`#wzReportHome` → `gaReport()`). These are where Book Demo + Report went after they were pulled from the nav. The Willis bubble is always visible (incl. mobile), so both stay one click away.
